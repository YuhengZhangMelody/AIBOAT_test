...
from ui.ui_utils import _world_to_screen, draw_cardinal_labels_screen_edges, draw_fov_cone, draw_heading_line, _boat_to_world, _cam_to_boat
...
class UIVision:
    ...
    def update(self):
        ...
        x_black = -0.36*2
        y_black = 3
        B = {"ObjId":1, "IdCam":config.black_id_cam, 'x':x_black, 'y':z_black, 'z':-y_black, 'dist':math.sqrt(x_black*x_black + y_black*y_black + z_black*z_black)}
        z_grey = 2
        x_grey = 0
        y_grey = 3
        G = {"ObjId":2, "IdCam":config.grey_id_cam, 'x':x_grey, 'y':z_grey, 'z':-y_grey, 'dist':math.sqrt(x_grey*x_grey + y_grey*y_grey + z_grey*z_grey)} 
        
        self.boat_state["detected_objects"] = [B, G]

        self.boat_state["heading"] += math.radians(1)  # rotation continue pour tester l'animation du FOV
        """
    ...
    def render(self, screen: pygame.Surface):
        ...
        points = self.boat_state["detected_objects_boat"]
        if points is None:
            points = []
        ...
        draw_heading_line(
            screen,
            heading_rad=math.pi/2 -heading, # +math.pi/2 pour compenser le fait que 0° FOV = +y écran
            length_px=100.0,
            origin_px=origin,
            color=(255, 0, 0)
        )

        draw_cardinal_labels_screen_edges(screen, margin_px=20)
        ...
        for cam in self.boat_state["Cameras"].values():
            ...
            # position caméra en repère écran
                ...
                print("Erreur à l'affichage : ", e)

        for obj in points:

            #position objet en repère bateau
            Xobj_boat, Yobj_boat = obj['x'], obj['y']
